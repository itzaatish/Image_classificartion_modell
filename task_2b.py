...
def classify_event(image):
    ...
    loaded_model = torch.load('trained_model_mobilenet_03.pth' ,map_location='cpu')
    output =loaded_model(image)
    output = torch.argmax(nn.functional.softmax(output , dim = 1), dim=1)
    class_list = ["combat","destroyedbuilding","fire","humanitarianaid","militaryvehicles"]
    event = class_list[output]
    return event
